...
export const GetAdvertisementList =
  (categoryId: string) => async (dispatch: Dispatch<AdvertisementActions>) => {
    try {
      const resp = await http.get<ISimpleServerResponse>(
        `/api/ticket/tickets?categoryId=${categoryId}`
      );
      const { data } = resp;
...
export const RemoveAdvertisement =
  (id: string) => async (dispatch: Dispatch<AdvertisementActions>) => {
    try {
...
export const UpdateAdvertisement =
  (ad: IAdvertisement) => async (dispatch: Dispatch<AdvertisementActions>) => {
    try {
      // to do
